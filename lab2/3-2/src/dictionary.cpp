...
const char SPACE = ' ';
const char FIRST_CHAR_IN_ALF = 'a';
const char LAST_CHAR_IN_ALF = 'z';

bool IsEnglandString(const std::string& string)
{
    return std::all_of(string.begin(), string.end(),
                       [](char c) { return (c >= FIRST_CHAR_IN_ALF && c <= LAST_CHAR_IN_ALF) || c == SPACE; });
}
...
void SaveDict(std::ofstream& outputFile, std::map<std::string, std::vector<std::string>> dict)
{
